...
/* ========================================================================
   ...
   For a project task specifically, write access has three levels:
     'none'    — a project member who's neither the creator nor the
                 assignee: read-only (or no access at all, if not even a
                 project member — see canReadTask).
     'limited' — the task's assignee, when that's someone other than
                 whoever created/assigned it: can update status, checklist
                 items, and developer notes, and can attach new
                 ...
                 disappear.
     'full'    — admin, OR the task's creator. Creating/assigning a task
                 doesn't cost you the ability to edit it just because you
                 handed it to someone else — every field, delete included.
   ==================================================================== */
...
function taskWriteLevel(task, user, projectsById) {
  ...
  if (type === 'project') {
    // The task's creator — the one who assigned it in the first place, and
    // the only non-admin who could ever reassign it — keeps full control
    // even after handing it to someone else; creating/assigning a task
    // isn't supposed to cost you the ability to edit it. The assignee (if
    // a different person) gets the narrower 'limited' tier.
    if (task.createdById && task.createdById === user.id) return 'full';
    return task.assigneeId === user.id ? 'limited' : 'none';
  }
  return 'full';
}
...
// Records who performed an assignment — stamped server-side from the
// AUTHENTICATED session user, never trusted from the client, since the
// whole point is an accountability trail (a client could otherwise claim
// "assigned by Admin" for an assignment it made itself). Only reachable
// from the full-write branch below: a limited-write assignee can't change
// assigneeId at all (it's not in LIMITED_WRITE_FIELDS), so they can never
// stamp themselves as the assigner of their own task.
function stampAssignment(task, previous, user) {
  const prevAssignee = previous ? (previous.assigneeId || '') : '';
  const nextAssignee = task.assigneeId || '';
  if (!nextAssignee) return { ...task, assignedById: '', assignedByName: '' }; // unassigned — no assigner to record
  if (nextAssignee === prevAssignee) {
    // Unchanged assignment — never trust whatever the client echoed back
    // for these two fields either; keep the server's own prior record so
    // the trail can't be rewritten as a side effect of an unrelated edit.
    return { ...task, assignedById: previous ? (previous.assignedById || '') : '', assignedByName: previous ? (previous.assignedByName || '') : '' };
  }
  return { ...task, assignedById: user.id, assignedByName: user.name || '' };
}
...
export async function writeDbForUser(user, incoming) {
  ...
  const sanitizedIncomingTasks = incomingTasks
    ...
    .map(t => {
      ...
      // one.
      if (existing) return canFullyWriteTask(existing, user, projectsById) ? stampAssignment(t, existing, user) : existing;
      // Brand-new task.
      return canCreateTask(t, user, projectsById) ? stampAssignment(t, null, user) : null;
    })
    .filter(Boolean);
